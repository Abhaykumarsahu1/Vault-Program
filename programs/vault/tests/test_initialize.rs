...
fn test_initialize() {
    ...
    let counter_state = vault::state::Counter::try_deserialize(&mut data).unwrap();
    assert_eq!(counter_state.count, 0);
    assert_eq!(counter_state.authority, payer.pubkey());

    let instruction = Instruction::new_with_bytes(
        program_id,
        &vault::instruction::Increment {}.data(),
        vault::accounts::Increment {
            counter,
